...
class Experiment:
    def __init__(self, num_iterations, batch_size, lr, dr, input_drop, dropout, dropout_3d, padding, dembd, dembd1, max_ary):
        self.num_iterations = num_iterations
        self.batch_size = batch_size
        self.lr, self.dr = lr, dr
        self.input_drop, self.dropout, self.dropout_3d = input_drop, dropout, dropout_3d
    ...
    def train_and_eval(self):
        ...
        model = HyCRE(len(d.ent2id), len(d.rel2id), self.input_drop, self.dropout, self.dropout_3d, self.padding, self.dembd, self.dembd1, self.max_ary, self.device)
        model.to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default="JF17K-4", nargs="?", help="FB-AUTO/JF17K/WikiPeople/WN18RR/FB15K-237.")
    ...
    parser.add_argument("--lr", type=float, default=0.00080, nargs="?", help="Learning rate.")
    parser.add_argument("--dr", type=float, default=0.999, nargs="?", help="Decay rate.")
    
    parser.add_argument("--input_drop", type=float, default=0.9, nargs="?", help="input_drop.")
    parser.add_argument("--dropout", type=float, default=0.9, nargs="?", help="dropout.")
    parser.add_argument("--dropout_3d", type=float, default=0.9, nargs="?", help="dropout_3d.")
    
    parser.add_argument("--padding", type=int, default=3, nargs="?", help="Convolutional padding (conv_kernel size).")
    ...
    param['input_drop'], param['dropout'], param['dropout_3d'] = args.input_drop, args.dropout, args.dropout_3d
    ...
    Exp = Experiment(num_iterations=param['num_iterations'], batch_size=param['batch_size'], lr=param['lr'], dr=param['dr'],
                     input_drop=param['input_drop'], dropout=param['dropout'], dropout_3d=param['dropout_3d'], padding=param['padding'], 
